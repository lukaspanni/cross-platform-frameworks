...
Anhand der Umsetzung der Unterstützung der plattformfremden Programmiersprache wird dieser Ansatz weiter unterteilt.
Die plattformfremden Programmiersprachen werden entweder über Interpretation des Codes (\textit{Interpreted App}), Ausführung in einer virtuellen Maschine auf der Zielplattform (\textit{VM App}) oder Generierung von Code in der Plattformspezifischen Programmiersprache (\textit{Generated App}) unterstützt.
Auch eine Mischung der Ansätze ist hierbei denkbar \cite{Nunkesser_Taxonomy_Apps}.
Populäre Frameworks wie Xamarin und Flutter gehören zur Kategorie der Foreign Language Apps und ermöglichen die Entwicklung mit C\# respektive Dart, welche weder von Android noch von iOS für die Entwicklung von Apps vorgesehen sind \cite{Xamarin_Homepage} \cite{Fentaw_Thesis_Flutter}.
...
Demnach wird im Folgenden zwischen nativen Apps beziehungsweise Endemic Apps, Web-Apps und Cross-Plattform Apps unterschieden.
...
Dennoch sind sowohl Framework-Entwickler \cite{Xamarin_Homepage, Kremer_IonicROI} als auch unabhängige Literatur \cite{Pinto_Native_to_Cross_Platform, Que_Comparison_Hybrid_Native, Nawrocki_Comparison_Hybrid_Native_Frameworks} einig, dass eine Cross-Plattform Entwicklung Kostenvorteile mit sich bringt.
...
Zum Beispiel verwendet React Native jeweils native \ac{UI}-Elemente, sodass die Benutzeroberfläche dem Look-and-Feel der Plattform entspricht \cite{React_NativeComponents}.
